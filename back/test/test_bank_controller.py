import unittest
from utils.exception import InvalidTakeTokenAction
...
class Test_BankController(unittest.TestCase):
    ...
    def test_withdraw(self):
        ...
        bc.withdraw(TokenArray([1, 1, 1, 0, 0, 0]), TokenArray([0, 0, 0, 0, 0, 0]))
        self.assertEqual(bc.bank.get_tokens(), [6, 6, 6, 7, 7, 5])
        bc.withdraw(TokenArray([2, 0, 0, 0, 0, 0]), TokenArray([0, 0, 0, 0, 0, 0]))
        self.assertEqual(bc.bank.get_tokens(), [4, 6, 6, 7, 7, 5])
        self.assertEqual(bc.withdraw(TokenArray([2, 0, 0, 0, 0, 0]), TokenArray([0, 0, 0, 0, 0, 0])), None)
        self.assertEqual(bc.bank.get_tokens(), [2, 6, 6, 7, 7, 5])
        self.assertIsInstance(bc.withdraw(TokenArray([0, 0, 0, 0, 0, 1]), TokenArray([0, 0, 0, 0, 0, 0])), InvalidTakeTokenAction)

        bc.withdraw(TokenArray([1, 1, 1, 0, 0, 0]), TokenArray([0, 0, 0, 0, 0, 0]))
        bc.withdraw(TokenArray([1, 1, 1, 0, 0, 0]), TokenArray([0, 0, 0, 0, 0, 0]))
        self.assertIsInstance(bc.withdraw(
            TokenArray([1, 1, 1, 0, 0, 0]), TokenArray([0, 0, 0, 0, 0, 0])), InvalidTakeTokenAction)
